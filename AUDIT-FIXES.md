...
- **Quran āyah (`.aya-block`):** al-Baqarah 2:179, PASTE-only byte-exact from quran.com imlaei (api.quran.com/api/v4/quran/verses/imlaei?verse_key=2:179). Voweled, never retyped. Verified `text_imlaei == in-file block-ar` (True). Note: an initial Write normalized shadda/fatha order; corrected by re-pasting the canonical string → now byte-equal to source.
- **No standalone hadith-block.** The Ḥārith b. Suwayd / al-Mujadhdhar report is referenced only inside Ibn al-Qayyim's THAI-translated quotation (no Arabic text given in source) → no `.hadith-block` created (R8: nothing to paste).
- **Scholar Arabic (`.ar-quote`) — R60 ALL-OR-NOTHING → uniformly BARE:** 2 blocks (Ibn Juzayy al-Maliki; Ibn Abi Zayd al-Qayrawani). Source already supplied both bare (0 harakat); reproduced byte-exact, no stripping needed. Verified harakat count 0 in both; skeleton byte-equal to source. Ibn Juzayy block retains source's Arabic comma ، (U+060C) — correct per R77, untouched.
- No Arabic punctuation normalization performed (source scholar Arabic already R77-correct). No splice, no skeleton retype.


## 2026-06-21 — NEW BUILD: polygamy-encouraged-or-permitted (nitisart) — Arabic provenance
New article (not a retro-audit). Source: Drive `1NMMf2_2LLrucw7aM07BXE6DSmQuI3cNrfWvLLXegzqk` ("#มีภรรยาหลายคนเป็นสิ่งที่ศาสนาส่งเสริมหรือแค่อนุญาต"). R71 child-safety screen: none (adult marriage fiqh — polygamy as sunnah vs mubāḥ; no minors, no marriage-age, no sexualized content).
- **21 scholar `.ar-quote` blocks** (al-Qurṭubī/al-Mufhim; al-Mardāwī ×2 [Taṣḥīḥ al-Furūʿ, al-Inṣāf]; al-Hidāyah; al-Ḥajjāwī; al-Sharwānī; al-Najm al-Wahhāj; al-Khaṭīb al-Shirbīnī; al-Shāfiʿī; Mukhtaṣar al-Muzanī; al-Māwardī/al-Ḥāwī ×2; al-ʿImrānī/al-Bayān debate ×4; al-Raymī; al-Zuḥaylī ×2; al-Shanqīṭī; al-Mawsūʿah al-Kuwaytiyyah) stripped to uniformly BARE per Rule 60 (source was MIXED voweled+bare, so all-or-nothing → BARE). Delete-only U+064B–U+0652; skeleton + spaces UNCHANGED. Byte-diff verified: each block's bare skeleton == the Drive source line with only harakat removed (21/21 byte-exact match; 0 harakat remaining in any ar-quote). 665 harakat marks removed total. sha1(concat of 21 blocks)=eed47bdc3178f598. File blob SHA=2ad4460edf5fdf836b9f271f175e6c1fe6a2f46f. Never retyped.
- **NO standalone `.aya-block` / `.hadith-block` in this article.** Every Arabic passage is a scholar quotation.
